Ext.define('isane.controller.aq_yb_lr_hb.hbPanel', {
	extend : 'Ext.app.Controller',
	stores : ['aq_yb_lr_hb.hbTree'],
	models : ['organtree'],	
	views : ['aq_yb_lr_hb.hbPanel', 'aq_yb_lr_hb.hbWest', 'aq_yb_lr_hb.hbList', 'aq_yb_lr_hb.importForm'],
	init: function() {
		this.control({
			//hb
			'aq_yb_lr_hb-hbPanel':{
    		...
    		'aq_yb_lr_hb-hbList button[text=搜索]':{
				click:this.click_search
			},			
			'aq_yb_lr_hb-hbList button[text=保存]':{
				click: this.saveAll
			},				
    ...
    itemclick_dt: function(own, record, item, index, e, eOpts){
		...
		Ext.getCmp('aq_yb_lr_hb-hbList-organCode').setValue(organCode);
		Ext.getCmp('aq_yb_lr_hb-hbList-searchButton').setDisabled(false);
		Ext.getCmp('aq_yb_lr_hb-hbList-saveButton').setDisabled(false);		
		Ext.getCmp('aq_yb_lr_hb-hbList-importButton').setDisabled(false);		
		Ext.getCmp('aq_yb_lr_hb-hbList-refresh').setDisabled(false);		
	...
	afterrender: function(panel){
		...
		var storeD = Ext.getCmp('aq_yb_lr_hb-hbList-storeD').getValue();
		var organCode = Ext.getCmp('aq_yb_lr_hb-hbList-organCode').getValue();
	...
	importBtn: function(btn){
		...
		var win = Ext.create('Ext.window.Window',{
			...
			modal: true,
			border: 0,
			items: [{xtype: 'aq_yb_lr_hb-importForm'}],
		...
		Ext.getCmp('aq_yb_lr_hb-importForm-storeDate').setValue(storeDate);
		win.show();		
	},
