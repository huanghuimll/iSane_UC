Ext.define('isane.controller.aq_kjgl_lwtj.lwtjPanel', {
	...
	init: function() {
		this.control({
			...
			'aq_kjgl_lwtj-lwtjList button[text=明细]': { 
				click : this.click_view
			}, 
			'aq_kjgl_lwtj-lwtjList button[text=修改]':{
				click: this.click_update
			},
			'aq_kjgl_lwtj-lwtjList button[text=删除]':{
				click: this.click_delete
			},			
			'aq_kjgl_lwtj-lwtjList button[text=搜索]':{
	...
	itemclick: function(grid){
		...
		//console.log(bts);
		Ext.getCmp('aq_kjgl_lwtj-lwtjList-viewButton').setDisabled(false);
		Ext.getCmp('aq_kjgl_lwtj-lwtjList-editButton').setDisabled(false);
		Ext.getCmp('aq_kjgl_lwtj-lwtjList-removeButton').setDisabled(false);
	},
	...
	click_add: function(){
		...
		var win = Ext.create('Ext.window.Window',{
			...
			iconCls: 'list_add',
			border: 0,
			items: [{xtype: 'aq_kjgl_lwtj-lwtjForm'}],
