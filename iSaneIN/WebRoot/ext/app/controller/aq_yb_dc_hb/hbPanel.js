Ext.define('isane.controller.aq_yb_dc_hb.hbPanel', {
	...
	views : ['aq_yb_dc_hb.hbPanel'],
	init: function() {
		this.control({
			//hb月报导出
			'aq_yb_dc_hb-hbPanel button[text=导出]':{
				click: this.exportBtn
			}, 
    		'aq_yb_dc_hb-hbPanel button[text=搜索]':{
	...
	exportBtn: function(btn){
		var organCode = Ext.getCmp('aq_yb_dc_hb-hbPanel-organCode').getValue();
		var storeY = Ext.getCmp('aq_yb_dc_hb-hbPanel-storeY').getValue();
		var storeM = Ext.getCmp('aq_yb_dc_hb-hbPanel-storeM').getValue();
		var storeD = Ext.getCmp('aq_yb_dc_hb-hbPanel-storeD').getValue();
		var storeDate = storeY + '-' + QJ_UtilEntity.month(storeM) + '-'+ QJ_UtilEntity.month(storeD);
		var tempName = null;
		var fileName = null;
		if(organCode = 'GZFGS'){
			tempName = 'GZFGS_Y_HB_REPORT';
			fileName = '广东分公司_环保月报统计';
		}
	...
	click_openHtml: function(btn){
		...
		Ext.Ajax.request({
			...
			success: function(response){
				...
				//console.log(html);
				var htmlPanel = Ext.getCmp('aq_yb_dc_hb-hbPanel-html');
