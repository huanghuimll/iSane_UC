Ext.define('isane.controller.aq_yb_dc_nh.nhPanel', {
	...
	views : ['aq_yb_dc_nh.nhPanel'],
	init: function() {
		this.control({
			//zh月报导出
			'aq_yb_dc_nh-nhPanel button[text=导出]':{
				click: this.exportBtn
			},
    		'aq_yb_dc_nh-nhPanel button[text=搜索]':{
	...
	exportBtn: function(btn){
		var organCode = Ext.getCmp('aq_yb_dc_nh-nhPanel-organCode').getValue();
		var storeY = Ext.getCmp('aq_yb_dc_nh-nhPanel-storeY').getValue();
		var storeM = Ext.getCmp('aq_yb_dc_nh-nhPanel-storeM').getValue();
		var storeD = Ext.getCmp('aq_yb_dc_nh-nhPanel-storeD').getValue();
	...
	click_openHtml: function(btn){
		...
		Ext.Ajax.request({
			...
			success: function(response){
				...
				//console.log(html);
				var htmlPanel = Ext.getCmp('aq_yb_dc_nh-nhPanel-html');
