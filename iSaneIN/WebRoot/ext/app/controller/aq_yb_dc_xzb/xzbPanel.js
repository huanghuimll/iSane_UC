Ext.define('isane.controller.aq_yb_dc_xzb.xzbPanel', {
	...
	//stores : ['aq_yb_dc_xzb.rbTree'],
	//models : ['organtree'],	
	views : ['aq_yb_dc_xzb.xzbPanel', 'aq_yb_dc_xzb.xzbList'],
	init: function() {
		this.control({
			//xzb月报导出
			'aq_yb_dc_xzb-xzbList button[text=导出]':{
				click: this.exportBtn
			},
			/*'aq_yb_dc_xzb-xzbPanel':{
				beforerender: this.onBeforeRender
			},				
			'aq_yb_dc_xzb-rbWest':{
    			itemclick: this.itemclick_dt
    		},
    		'aq_yb_dc_xzb-xzbList button[text=搜索]':{
				click:this.click_search
			},			
			'aq_yb_dc_xzb-xzbList button[text=保存]':{
				click: this.saveAll
			},				
			'aq_yb_dc_xzb-xzbList actioncolumn':{
	...
	exportBtn: function(btn){
		var organCode = Ext.getCmp('aq_yb_dc_xzb-xzbList-organCode').getValue();
		var storeY = Ext.getCmp('aq_yb_dc_xzb-xzbList-storeY').getValue();
		var storeM = Ext.getCmp('aq_yb_dc_xzb-xzbList-storeM').getValue();
		var storeD = Ext.getCmp('aq_yb_dc_xzb-xzbList-storeD').getValue();
	...
	onBeforeRender: function(item){
		var own = Ext.getCmp('aq_yb_dc_xzb-rbWest-id');
    ...
    itemclick_dt: function(own, record, item, index, e, eOpts){
		...
		Ext.getCmp('aq_yb_dc_xzb-xzbList-organCode').setValue(organCode);
		
		var grid = Ext.getCmp('aq_yb_dc_xzb-xzbList-id');
	...
	afterrender: function(panel){
		...
		var obt = {
				plantCode: Ext.getCmp('aq_yb_dc_xzb-xzbList-organCode').getValue(),
				dataType: 'RB_PT',
